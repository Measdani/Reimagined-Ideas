...
export function buildInquiry(payload: Record<string, unknown>): InquiryPayload {
  return {
...
export function createStoredInquiry(inquiry: InquiryPayload): StoredInquiry {
  return {
    ...inquiry,
    id: randomUUID(),
...
export function buildTextEmail(inquiry: InquiryPayload) {
  return [
...
export function buildHtmlEmail(inquiry: InquiryPayload) {
  const rows = [
    ["Name", inquiry.contactName],
    ["Organization", inquiry.organizationName],
    ["Email", inquiry.email],
